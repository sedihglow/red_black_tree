...
static uint32_t place_data(rbTree *tree, const int32_t toAdd);
...
static uint32_t count_data(rbNode *node, const int32_t toCount);
...
static rbNode* retrieve_match(rbNode *node, const int32_t toFind);
...
uint32_t place_data(rbTree *tree, const int32_t toAdd)/*#{{{*/
{
    uint32_t lastTurn;
...
uint32_t remove_first(rbTree *tree, const int32_t toRemove)/*#{{{*/
{
...
uint32_t count_data(rbNode *node, const int32_t toCount)/*#{{{*/
{
...
rbNode* retrieve_match(rbNode *node, const int32_t toFind)/*#{{{*/
{
    if(node == NULL){
        return NULL;}
...
uint32_t display_rbTree(rbNode *node)/*#{{{*/
{
    ...
    printf("data: %d, flag: %d, parent: %d\n",*node -> data, node -> cflag,
                            (node -> parent) ? *node -> parent -> data : 0);
